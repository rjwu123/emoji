...
class Emoji
{
    ...
    public $tagTemplate = "[emoji]{{}}[/emoji]" ;
    ...
    private function bin_to_unicode($bin)
    {
        ...
        if ($num > 0xffffff) {
             #四个字节
            $unicode_num = ((ord($bin[0]) & 0x7) << 18) | ((ord($bin[1]) & 0x3f) << 12) | ((ord($bin[2]) & 0x3f) << 6) | (ord($bin[3]) & 0x3f);
        } else if ($num > 0xffff) {
             #三个字节
            $unicode_num = ((ord($bin[0]) & 0xf) << 12) | ((ord($bin[1]) & 0x3f) << 6) | (ord($bin[2]) & 0x3f);
        } else if ($num > 0xff) {
             #两个字节
            $unicode_num = ((ord($bin[0]) & 0x1f) << 6) | (ord($bin[1]) & 0x3f);
        } else {
            $unicode_num = ord($bin);
        }
